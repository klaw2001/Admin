...
const BasicTable = ({ products, onView, onEdit, onDelete, openModal , openImageModal }) => {
  const columns = [
    {
      name: (
        <Input
    ...
    },
    {
        ...
        cell: (row) =>
          row.images && row.images.length > 0 ? (
            <div onClick={() => openImageModal(row.images[0])}>
              <img
                src={row?.images[0]}
                alt="Thumbnail"
                style={{ maxWidth: "50px", cursor: "pointer" }}
  ...
  const data = products?.map((product, index) => ({
    ...
    id: product?._id,
    thumbnail: product?.thumbnail || "no img",
    name: product?.name,
    category: product?.category?.name,
    price: `$${product?.price?.toFixed(2)}` || "No Price Given",
